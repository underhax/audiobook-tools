...
func TestReplaceWindows(t *testing.T) {
	...
	validArchive := createDummyZip(t, "audiobook-tools.exe", dummyContent)
	emptyArchive := createDummyZip(t, "not-audiobook-tools.exe", dummyContent)
	...
	tests := []struct {
		...
		{
			name:        "missing_audiobook_tools_win",
			body:        bytes.NewBuffer(emptyArchive),
			wantErr:     true,
			errContains: "audiobook-tools.exe not found in archive",
		},
		{
		...
		},
		{
			...
			wantErr:     true,
			errContains: "failed to open audiobook-tools.exe in zip",
		},
		{
			name: "close_error",
			body: bytes.NewBuffer(validArchive),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := replaceWindows(tt.body, "/test/audiobook-tools.exe")
...
func TestCleanupWindowsOldFiles(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "symlink_error",
			mockExecutable: func() (string, error) {
				return "/test1/audiobook-tools.exe", nil
			},
		...
		},
		{
			name: "success",
			mockExecutable: func() (string, error) {
				return "/test2/audiobook-tools.exe", nil
			},
		...
		},
		{
			name: "remove_error",
			mockExecutable: func() (string, error) {
				return "/test3/audiobook-tools.exe", nil
			},
			mockFilepathEval: func(s string) (string, error) {
				return s, nil
			},
...
func TestDefaultZipOpen(t *testing.T) {
	validArchive := createDummyZip(t, "audiobook-tools.exe", "content")
