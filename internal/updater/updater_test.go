...
func TestUpdate(t *testing.T) {
	...
	validArchivePOSIX := createDummyTarGz(t, "audiobook-tools", dummyContent)
	validArchiveWin := createDummyZip(t, "audiobook-tools.exe", dummyContent)
	...
	validJSON := fmt.Sprintf(`{
		"tag_name": "v1.1.0",
		"assets": [
			{"name": "audiobook-tools-linux-amd64.tar.gz", "browser_download_url": "http://dl/linux-amd64", "digest": "%s"},
			{"name": "audiobook-tools-linux-arm64.tar.gz", "browser_download_url": "http://dl/linux-arm64", "digest": "%s"},
			{"name": "audiobook-tools-macos-intel.tar.gz", "browser_download_url": "http://dl/macos-intel", "digest": "%s"},
			{"name": "audiobook-tools-macos-apple-silicon.tar.gz", "browser_download_url": "http://dl/macos-silicon", "digest": "%s"},
			{"name": "audiobook-tools-windows-x64.zip", "browser_download_url": "http://dl/windows-x64", "digest": "%s"},
			{"name": "audiobook-tools-windows-arm64.zip", "browser_download_url": "http://dl/windows-arm64", "digest": "%s"}
		]
	...
	tests := []updateTestCase{
		...
		},
		{
			...
			client: successClient,
			mockExecutable: func() (string, error) {
				return "/bin/audiobook-tools", nil
			},
		...
		},
		{
			...
			mockOS: osWindows,
			mockExecutable: func() (string, error) {
				return filepath.Join(t.TempDir(), "audiobook-tools.exe"), nil
			},
		...
		},
		{
			...
			mockOS: "linux",
			mockExecutable: func() (string, error) {
				return filepath.Join(t.TempDir(), "audiobook-tools"), nil
			},
			mockChmod:  func(_ string, _ os.FileMode) error { return nil },
			mockRename: func(_, _ string) error { return nil },
			mockRemove: func(_ string) error { return nil },
