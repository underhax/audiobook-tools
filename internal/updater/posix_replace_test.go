...
func TestReplacePOSIX(t *testing.T) {
	...
	validArchive := createDummyTarGz(t, "audiobook-tools", dummyContent)
	...
	emptyArchive := createDummyTarGz(t, "not-audiobook-tools", dummyContent)
	...
	tests := []struct {
		...
		{
			name:        "missing_audiobook_tools_posix",
			body:        bytes.NewBuffer(emptyArchive),
			wantErr:     true,
			errContains: "audiobook-tools executable not found in archive",
		},
		{
		...
		{
			name: "chmod_error",
			body: bytes.NewBuffer(createDummyTarGz(t, "audiobook-tools", dummyContent)),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := replacePOSIX(tt.body, "/test/audiobook-tools")
...
func TestReplacePOSIXFull(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "missing_audiobook_tools_full",
			body:        bytes.NewBuffer(emptyArchive),
			wantErr:     true,
			errContains: "audiobook-tools executable not found in archive",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := replacePOSIX(tt.body, filepath.Join(tempDir, "audiobook-tools"))
			if (err != nil) != tt.wantErr {
				t.Errorf("replacePOSIX() error = %v, wantErr %v", err, tt.wantErr)
			}
			if err != nil && tt.errContains != "" {
...
func TestDefaultTempFileClose(t *testing.T) {
	tempFile, err := os.CreateTemp("", "audiobook-tools-test-close-*")
	if err != nil {
